Feature: Nitro's Customs (vehicle weapon)
  Nitro's Customs launches a wave of large Rust-style cars that sweep across the player's screen
  in a straight line, piercing every fish in their lane for damage. Each level adds one more car
  (count = damage = level). Its evolution, Nitro's Dealership, fields seven cars at once and is
  gated behind Nitro's Customs at Lv5 plus Subversive Sybex at max stack.
  ...
  Scenario: Level 5 launches five cars at once
    Given a player "Driver" at (4000, 4000) with mass 50
    And "Driver" has weapon "nitros" at level 5
    When the world advances 1 tick
    Then 5 vehicle bodies owned by "Driver" are in flight

  Scenario: Nitro's Dealership fields seven cars at once
    Given a player "Baller" at (4000, 4000) with mass 50
    And "Baller" has weapon "dealership" at level 1
    When the world advances 1 tick
    Then 7 vehicle bodies owned by "Baller" are in flight
  ...
  Scenario: Cars sweep across and despawn within their lifetime
    ...
    When the world advances 1 tick
    Then 1 vehicle bodies owned by "Driver" are in flight
    # lifetime 3.8s < 5s, comfortably inside the 14s cooldown.
  ...
  Scenario: The next wave only launches after the cooldown
    ...
    When the world advances 1 tick
    Then 5 vehicle bodies owned by "Driver" are in flight
    # past the cars' 3.8s lifetime but inside the 10s cooldown → none in flight.
    When the world advances 5 seconds
    Then 0 vehicle bodies owned by "Driver" are in flight
