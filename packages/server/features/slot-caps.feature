Feature: Combined slot cap
  A fish can hold at most 5 total skills — any mix of weapons and passives.
  Once five slots are used, no new weapon and no new passive may be added;
  ...
  Scenario: With 5 slots used, no new weapon is offered
    ...
    And "Alpha" has weapon "ink" at level 1
  ...
  Scenario: With 5 slots used, no new passive is offered
    Given a player "Alpha" at (1000, 1000) with mass 10
    And "Alpha" has weapon "bubble" at level 1
    And "Alpha" has weapon "spine" at level 1
    And "Alpha" has passive "fin" at stack 1
    And "Alpha" has passive "gulp" at stack 1
    And "Alpha" has passive "recovery" at stack 1
    And "Alpha" has accumulated 100 XP
    When level-ups are processed
    Then "Alpha"'s pending cards do not stack a new passive

  Scenario: Fully maxed 5-slot loadout levels silently with no modal
