Feature: Title screen
  The lobby gate: pick a name + fish species, then go deep. Keeps the entry flow
  ...
  Scenario: Title overlay renders with name input and GO DEEP
  ...
  Scenario: GO DEEP with a name sends a hello with that name
