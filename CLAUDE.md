...
Fish-vs-fish eating (in `world.step`, `packages/server/src/sim/world.ts`) is **mouth-on, front-cone**: a fish swallows edible prey (`canSwallow`) only when the prey sits in its forward mouth cone (`MOUTH.coneCos`) **and** the body-edge gap `dist − rA − rB` is within `MOUTH.eatReach` — a flat **~5px** ("get your mouth on the fish"), multiplied by the Close Encounters passive for players (`getEatRangeMult`), flat for AI. There is **no suction and no behind-approach bonus** (the old `suctionExtraRadius`/`reachBonus`/`behindReachBonus`/`contactMargin` are gone) — a chase lands only by physically closing to within the reach. A truly stationary fish (no heading) has no cone and eats on contact from any angle. While a predator that *could* swallow prey is facing it but still closing in (`eatReach < gap ≤ eatReach × MOUTH.biteReachMult` ≈ **20px**), it plays a cosmetic **bite wind-up** — sets `bitingTick`, pulsed by `BITE.cooldownMs` via `lastBiteAnimAt` — so prey see the chomp coming before the swallow. A smaller fish **nibbles** a bigger one (any angle, `gap ≤ eatReach`); a bigger fish that can't yet swallow takes a front **between-zone bite**. Freshly (re)spawned players carry `spawnProtectedUntil` (`SPAWN.protectMs`) so they can't be eaten/bitten in the window. On a swallow/bite the eater sets `bitingTick`/`nibblingTick` (snapshot → `EntityDelta.biting`/`nibbling`) and lunges forward: a real one-shot impulse (`BITE.eatLungeImpulse`/`lungeImpulse`) applied client-side in `stepSelf` for the player's own fish and server-side for AI eaters. The client runs `detectBites` (mirrors the server rules) to animate its **own** fish's chomp + predicted lunge instantly; the self fish ignores the snapshot `biting`/`nibbling` flags (`applyFishDelta` early-returns for self), while remote fish animate *from* those flags. **Combat toasts** (human victims only): `recordBite` enqueues `world.bittenEvents` once per attacker→victim engagement (per-attacker `Fish.biteToastAt` map + `BITE.toastEngagementMs`) → `index.ts` broadcasts `PlayerBittenMsg` → "X was bitten by Y"; the client also derives an "Ate `<name>`" toast from the snapshot `swallowed` array when it ate an AI fish (human victims are covered by the `playerDied` broadcast). **AI hunting note:** `AI.chaseSpeed` (235) sits just *below* `AI.fleeSpeed` (240), so edible prey that keep fleeing escape — with mouth-on eating AI mostly catch cornered/overlapping fish, by design.
